...
public class GuiChat extends net.minecraft.src.GuiChat {
	...
	String username;
	String tooltip = "";
	public static final String CHARS = "GSCLNWPU";
	boolean isBed;
	...
	public void drawScreen(int par1, int par2, float par3) {
		...
		fontRenderer.drawString(ColorCode.GREY + "Tip: Shift clicking will scroll 7x faster!", 220, height * 2 - 211, 0xFFFFFF);
		inputField.drawTextBox();
		GL11.glPopMatrix();
		int x = Mouse.getX();
		int y = Mouse.getY();
		int box = (x - 9) / ((219 - 28) / CHARS.length()) - 1;
		boolean is = y >= 202 && y <= 223 && x >= 28 && x <= 219;
	...
	public void handleMouseInput() {
		...
		int y = Mouse.getY();
		int box = (x - 9) / ((219 - 28) / CHARS.length()) - 1;
		if (y >= 202 && y <= 223 && x >= 28 && x <= 219) switch (box) {
			case 0:
